...
(структура перенесена, базовые эффекты уже работают в новом FX-слое)
...
- ✔ MATRIX RAIN (случайные "капли" яркости)
...
**Статус: ~70% (основные эффекты перенесены в `display_fx.c`, сложные сегментные эффекты пока в TODO).**
...
Реализованы базовые оверлеи (`boot`, `wifi`, `ntp`) в отдельном модуле `display_overlay.c`,
который работает поверх LL-буфера и не трогает FX.

**Статус: ~60% (базовые оверлеи перенесены, Error/расширенная FSM — TODO).**
...
Патч от 27.11.2025: перенесены основные эффекты и базовые оверлеи, добавлен MATRIX RAIN.