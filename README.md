

[![License](https://img.shields.io/github/license/DeadWacko/VFDDisplay?style=flat-square)](LICENSE)
[![Last commit](https://img.shields.io/github/last-commit/DeadWacko/VFDDisplay?style=flat-square)](https://github.com/DeadWacko/VFDDisplay/commits)
[![GitHub stars](https://img.shields.io/github/stars/DeadWacko/VFDDisplay?style=flat-square)](https://github.com/DeadWacko/VFDDisplay/stargazers)
[![GitHub issues](https://img.shields.io/github/issues/DeadWacko/VFDDisplay?style=flat-square)](https://github.com/DeadWacko/VFDDisplay/issues)

# VFDDisplay Library (v1.0.0)

**Библиотека для управления VFD (вакуумно-люминесцентными) дисплеями на базе микроконтроллера RP2040 (Raspberry Pi Pico).**

Проект реализует многослойную архитектуру драйвера с аппаратным мультиплексированием, защитой от паразитной засветки (anti-ghosting) и  движком процедурных анимаций.

---

## 🔥 Ключевые особенности

### Low-Level Driver (LL)
*   **Thread-Safe:** Атомарный доступ к буферам, отсутствие гонок данных и Deadlock-ов.
*   **Anti-Ghosting:** Реализован Dead-time (10 мкс) для предотвращения засветки соседних разрядов.
*   **Performance:** Развертка на базе `repeating_timer` и `hardware_alarm` (без загрузки CPU).
*   **Gamma Correction:** Аппаратная коррекция яркости ($x^2$).
*   **8-bit PWM:** Плавная регулировка яркости (256 уровней) для каждого разряда.

### High-Level Core (HL)
*   **FX Engine:** 10 встроенных эффектов (Matrix, Glitch, Marquee и др.).
*   **Transparency:** "Прозрачные" эффекты яркости накладываются поверх идущих часов.
*   **Text Engine:** Поддержка бегущей строки и вывода текста (Digits + Alpha).
*   **Overlay System:** Приоритетная система уведомлений (Boot, Wi-Fi, Errors).
*   **Smart Features:** Автояркость (ADC), ночной режим, мигание разделительных точек.

---

##  Подключение

Библиотека рассчитана на работу через сдвиговые регистры **74HC595** (каскад).

**Схема данных:**
1.  Первый регистр (или группа): **Сетки (Grids)**.
2.  Второй регистр (или группа): **Сегменты (Segments)**.

**Пины по умолчанию (RP2040):**
*   **DATA (DS):** GP15
*   **CLOCK (SH_CP):** GP14
*   **LATCH (ST_CP):** GP13
*   **ADC (Light Sensor):** GP26 (опционально)

---

##  Быстрый старт

### 1. Сборка
Проект использует стандартный **CMake**.

```bash
mkdir build
cd build
cmake ..
make
```

### 2. Пример использования (Main Loop)

Минимальный пример для запуска часов с эффектом "дыхания".

```c
#include "pico/stdlib.h"
#include "display_api.h"

int main() {
    stdio_init_all();
    
    // 1. Инициализация (4 разряда)
    display_init(4);
    
    // 2. Настройка
    display_set_brightness(255);
    display_set_dot_blinking(true);
    
    // 3. Запуск приветствия (Текст выезжает справа)
    display_fx_slide_in("HELLO", 150);
    
    // 4. Главный цикл
    while (true) {
        // ОБЯЗАТЕЛЬНО: Тик движка (Анимации, Яркость, Мультиплексинг)
        display_process();
        
        // Обновление времени (ваша логика)
        // display_show_time(hour, min, true);
        
        // Запуск эффекта по событию (например, дыхание)
        if (!display_is_effect_running()) {
             display_fx_pulse(5000);
        }

        sleep_ms(5);
    }
}
```

---

##  Система эффектов (FX)

Библиотека включает 3 типа эффектов:

1.  **Прозрачные (Transparent):** Меняют только яркость. Часы продолжают идти сквозь эффект.
    *   `Pulse`, `Wave`, `Scanner` (KITT), `Heartbeat`, `Fade In/Out`.
2.  **Блокирующие (Blocking):** Захватывают управление сегментами. Контент восстанавливается после завершения.
    *   `Glitch`, `Morph`, `Dissolve`, `Slot Machine`, `Decode`, `Ping-Pong`.
3.  **Текстовые:**
    *   `Marquee` (Бегущая строка), `Slide In`.

---

## 📚 Документация

Подробное описание архитектуры и API доступно в папке `docs/`:

*   [**API Reference**](docs/HL_API.md) — Описание всех функций для разработчика.
*   [**Low-Level Driver**](docs/LL_API.md) — Детали работы драйвера железа.
*   [**Architecture**](docs/ARCHITECTURE.md) — Структура слоев и потоков данных.
*   [**FX Engine**](docs/Effects_System.md) — Как работает движок эффектов.

---

## Лицензия

MIT License. Copyright (c) 2025 DeadWacko.
